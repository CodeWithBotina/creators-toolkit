...
if __name__ == "__main__":
    ...
    # Ensure theme is set to "dark-blue" in default_config here
    ConfigManager(config_dir="../config") 
    test_logger = get_application_logger()
    test_logger.info("--- Starting VideoBgRemovalPage isolated test ---")


    class DummyApp:
        """A minimal mock for the main App class to satisfy VideoBgRemovalPage's dependency."""
        def __init__(self):
            self.logger = get_application_logger()
    ...
    root.title("Video Background Removal Page Test")
    root.geometry("900x700") 
    root.grid_columnconfigure(0, weight=1)
    root.grid_rowconfigure(0, weight=1)

    dummy_app = DummyApp()
    bg_removal_page = VideoBgRemovalPage(root, dummy_app)
    bg_removal_page.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)

    # Automatically set a dummy input file for easier testing if available
    dummy_test_video = Path(__file__).parent.parent.parent / "test_media" / "test_video_for_bg_remove.mp4"
    if dummy_test_video.exists():
        bg_removal_page.input_file_path = dummy_test_video
        bg_removal_page._update_entry_text(bg_removal_page.input_entry, str(dummy_test_video))
        bg_removal_page._suggest_output_file_path()
        bg_removal_page._update_ui_state(True) # Enable process button since input is set
        test_logger.info(f"Pre-set dummy input video for testing: {dummy_test_video}")
    else:
        test_logger.warning(f"Dummy test video for background removal not found at {dummy_test_video}. Please create it or browse manually.")
        
    root.mainloop()
    test_logger.info("--- VideoBgRemovalPage isolated test completed ---")